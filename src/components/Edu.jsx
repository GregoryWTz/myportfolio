...
function EduEntry({ entry }) {
    const [cardRef, cardVisible] = useInView();
    const [subjectsRef, subjectsVisible] = useInView({ threshold: 0.1 });
    ...
    return (
        <Fragment>
            <div
                ref={cardRef}
                className={`edu__entry ${cardVisible ? "in-view" : ""}`}
            >
            ...
            <div
                ref={subjectsRef}
                className={`edu__subjects ${subjectsVisible ? "in-view" : ""}`}
            >
        ...
        <section id="edu" className="edu">
            <div className="container">
                ...
                {edu.entries.map((entry, index) => (
                    <EduEntry key={index} entry={entry} />
                ))}
            </div>
        </section>
    );
